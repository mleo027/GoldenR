...
public:
  ...
  void connect() {
    if (KGBPCli_StartX(handle_, config_.ip.c_str(), config_.port,
      ...
      throwBackendError("connect failed: " + config_.ip + ":" +
  ...
  void call(const NJSON &param, NJSON &result, const std::string &funcId) {
    ...
    if (param.contains("nodeid") && param["nodeid"].is_number()) {
      ...
      setBinaryOption(KGBPCLI_OPTION_NODE_ID, &nodeId, sizeof(nodeId),
                      "NODE_ID");
    }
    if (param.contains("clientsessionid") &&
        param["clientsessionid"].is_number()) {
      uint64_t clientSessionId =
          static_cast<uint64_t>(param["clientsessionid"].get<double>());
    ...
    int callRet = KGBPCli_CallProgram(
        handle_, const_cast<char *>(funcId.c_str()), config_.requestTimeoutMs);
...
private:
  ...
  void writeFields(const NJSON &param) {
    ...
    for (auto &item : param["fields"].items()) {
      ...
      if (value.is_binary()) {
        ...
        if (KGBPCli_SetValueByte(handle_, key.c_str(), bin.data(),
                                 bin.size()) != KGBPCLI_OK) {
          throwFieldError(key);
        }
        continue;
  ...
  int writeStringField(const std::string &key, std::string val) {
    ...
    if (val.size() > 6 && val.compare(0, 5, "file(") == 0 &&
        val.back() == ')') {
  ...
  void readReply(NJSON &result) {
    ...
    // 编码决策：KGBP 网关为 UTF-8，字段值直传不转码（区别于 KCBPClient 的
    // GBK→UTF8）
    int code = 0;
    char msg[1024] = {0};
  ...
  NJSON readResultSet() {
    ...
    if (KGBPCli_RsGetName(handle_, resultSetName, sizeof(resultSetName)) ==
            KGBPCLI_OK &&
        resultSetName[0] != '\0') {
      name = resultSetName;
    }
  ...
  // 使用手册 6.38：RsGetColInfo 的 apColInfo
  // 为「列名,列名,…」逗号隔开无空格的单个字符串。 解析顺序：①
  // 逗号分隔（文档权威格式）；② '\0' 分隔序列（兼容旧网关）；③ col1..colN
  // 兑底。 非首选路径触发时向 stderr 输出诊断（桥接进程透传），不再静默兑底。
  std::vector<std::string> parseColNames(size_t colNum) {
    ...
    if (KGBPCli_RsGetColInfo(handle_, buf.data(), buf.size() - 1) !=
        KGBPCLI_OK) {
    ...
    if (names.size() == colNum) {
      ...
      reportColInfoFallback("matched nul-separated (non-canonical)",
                            buf.data());
      return names;
    }
  ...
  splitColNamesByNul(const char *buf, size_t bufSize, size_t colNum) {
  ...
  void reportColInfoFallback(const std::string &reason, const char *buf) {
    ...
    std::fprintf(stderr,
                 "[KGBPClient] ColInfo fallback (%s) raw=", reason.c_str());
  ...
  static std::vector<std::string> splitColNamesByComma(const char *buf,
                                                       size_t colNum) {
    std::vector<std::string> names;
    if (buf == nullptr || colNum == 0 || *buf == '\0') {
      return names;
    ...
    while (names.size() < colNum) {
      ...
      size_t len =
          (comma != nullptr ? static_cast<size_t>(comma - p) : std::strlen(p));
      size_t begin = 0;
  ...
  std::string getColumnValue(int colIndex) {
    ...
    char buf[8192] = {0};
    if (KGBPCli_RsGetColByIndexStr(handle_, colIndex, buf, sizeof(buf)) !=
        KGBPCLI_OK) {
      return "";
    }
...
// 后端入口：签名与 callKCBPBackend 对齐，异常向上传播由 adapter.cpp 统一转
// -1003。
inline void callKGBPBackend(const NJSON &inputJson, NJSON &outputJson) {
  ...
  config.port =
      static_cast<uint16_t>(std::stoi(connection["port"].get<std::string>()));
  config.connectTimeoutMs = KGBPClientConfig::CONNECT_TIMEOUT_MS;
  config.requestTimeoutMs =
      std::stoi(connection["requesttimeout"].get<std::string>()) * 1000;
