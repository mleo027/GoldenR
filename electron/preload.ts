...
contextBridge.exposeInMainWorld('electronAPI', {
    readJsonFile: (filePath: string, fromUserData?: boolean) =>
        invoke<unknown>('readJsonFile', filePath, fromUserData),
    writeJsonFile: (filePath: string, data: unknown) =>
        invoke<void>('writeJsonFile', filePath, data),
    ...
    cancelKcbp: (): Promise<boolean> => invoke<boolean>('rpc:cancel'),
    kcbpRuntime: {
        getConfig: (): Promise<import('../src/shared/kcbp/types').KcbpRuntimeConfig> =>
            invoke<import('../src/shared/kcbp/types').KcbpRuntimeConfig>('kcbpRuntime:getConfig'),
        saveConfig: (
    ...
    openImportFile: (format: 'json' | 'ini') => invoke('import:openFile', format),
    openParamFile: () => invoke('param:openFile'),
    statParamFile: (filePath: string) => invoke('param:statFile', filePath),
    minimizeWindow: () => invoke('window:minimize'),
    toggleMaximizeWindow: (): Promise<boolean> => invoke('window:toggleMaximize'),
