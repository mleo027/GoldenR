...
export function migrateSchema(db: SqliteDatabase): void {
    db.transaction(() => {
        db.exec('CREATE TABLE IF NOT EXISTS schema_migrations (version INTEGER PRIMARY KEY, applied_at INTEGER NOT NULL); CREATE TABLE IF NOT EXISTS data_migrations (name TEXT PRIMARY KEY, applied_at INTEGER NOT NULL);');
        const row = db.prepare('SELECT MAX(version) AS version FROM schema_migrations').get() as { version?: number } | undefined;
        if (row?.version && row.version < SCHEMA_VERSION) { migrateV1(db); return; }
        if (!row?.version) { createSchema(db); db.prepare('INSERT INTO schema_migrations VALUES(?,?)').run(SCHEMA_VERSION, Date.now()); }
    })();
}
...
function createSchema(db: SqliteDatabase): void {
    db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS cases(id TEXT PRIMARY KEY,project_id TEXT NOT NULL,position INTEGER NOT NULL,name TEXT NOT NULL,protocol TEXT NOT NULL,address TEXT NOT NULL,favorite INTEGER NOT NULL DEFAULT 0,run_input_json TEXT NOT NULL DEFAULT '{}',created_at INTEGER NOT NULL,updated_at INTEGER NOT NULL,FOREIGN KEY(project_id) REFERENCES projects(id) ON DELETE CASCADE,UNIQUE(project_id,position));
      ...
      CREATE INDEX IF NOT EXISTS idx_cases_project_position ON cases(project_id,position); CREATE INDEX IF NOT EXISTS idx_common_params_set_position ON common_params(set_id,position); CREATE INDEX IF NOT EXISTS idx_environment_vars_environment_id ON api_debug_environment_vars(environment_id); CREATE INDEX IF NOT EXISTS idx_request_history_timestamp ON request_history(timestamp DESC);`);
}
...
function migrateV1(db: SqliteDatabase): void {
    ...
    const rows = (table: string): Record<string, unknown>[] => db.prepare(`SELECT * FROM ${table}`).all() as Record<string, unknown>[];
    const projects = rows('projects').map((r) => { const p = asRecord(parse(r.payload)); p.id = p.id ?? r.id; p.cases = rows('cases').filter((c) => c.project_id === r.id).map((c) => { const x = asRecord(parse(c.payload)); const cp = rows('case_params').find((q) => q.case_id === c.id); return { ...x, id: x.id ?? c.id, params: cp ? parse(cp.payload) ?? [] : x.params ?? [] }; }); return p; });
    const sets = rows('common_param_sets').map((r) => { const s = asRecord(parse(r.payload)); return { ...s, id: s.id ?? r.id, name: s.name ?? r.name, params: rows('common_params').filter((p) => p.set_id === r.id).map((p) => parse(p.payload) ?? {}) }; });
    const envs = rows('api_debug_environments').map((r) => { const e = asRecord(parse(r.payload)); e.id = e.id ?? r.id; e.vars = rows('api_debug_environment_vars').filter((v) => v.environment_id === r.id).reduce((a, v) => ({ ...a, [String(v.name)]: v.value }), {}); return e; });
    const readPayload = (table: string): unknown[] => rows(table).map((r) => parse(r.payload)).filter((v): v is unknown => v != null);
    const dbConfig = rows('db_connections')[0] ? parse(rows('db_connections')[0].payload) : null;
    const rules = readPayload('param_suggest_rules');
    const kcbp = rows('kcbp_runtime_config')[0] ? parse(rows('kcbp_runtime_config')[0].payload) : null;
    const history = rows('request_history').map((r) => parse(r.payload)).filter(Boolean);
    db.exec('DROP TABLE request_history; DROP TABLE kcbp_runtime_config; DROP TABLE param_suggest_rules; DROP TABLE db_connections; DROP TABLE api_debug_environment_vars; DROP TABLE api_debug_environments; DROP TABLE case_params; DROP TABLE cases; DROP TABLE projects; DROP TABLE common_params; DROP TABLE common_param_sets;');
    createSchema(db);
    const repo = new ConfigRepository(db);
    const app = db.prepare('SELECT value FROM app_preferences WHERE key=?').get('app.json') as { value?: string } | undefined;
    if (app) repo.write('app.json', parse(app.value));
    const settings = db.prepare("SELECT value FROM workspace_state WHERE key='snapshot'").get() as { value?: string } | undefined;
    ...
    db.prepare('UPDATE schema_migrations SET version=?, applied_at=? WHERE version<?').run(SCHEMA_VERSION, Date.now(), SCHEMA_VERSION);
}
