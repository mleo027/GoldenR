    ...
    it('接受合法的参数集文件结构', () => {
        expect(
            isCommonParamSetFile({
                sets: [{ id: 'a', name: '交易公共', params: [{ name: 'orgid', value: '0101', type: 'string' }] }],
            }),
        ).toBe(true);
    });
...
export function CommonParamsProvider({ children }: { children: ReactNode }) {
    ...
    const renameSet = useCallback(
        (setId: string, name: string) => {
            persist(
                setsRef.current.map((set) => (set.id === setId ? { ...set, name } : set)),
            );
        },
        [persist],
    );
    ...
    const updateSetParams = useCallback(
        (setId: string, params: CommonParamSet['params']) => {
            persist(
                setsRef.current.map((set) => (set.id === setId ? { ...set, params } : set)),
            );
        },
        [persist],
    );
