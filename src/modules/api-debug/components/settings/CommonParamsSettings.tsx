...
interface CommonParamDetailProps {
    active: CommonParamSet | null;
    onRename: (setId: string, name: string) => void;
    onUpdateParams: (setId: string, params: ParamItem[]) => void;
}

function CommonParamDetail({ active, onRename, onUpdateParams }: CommonParamDetailProps) {
    if (!active) {
        return (
                ...
                <div className="w-56 shrink-0 flex flex-col gap-2">
                    ...
                    <div className="flex flex-col gap-1 overflow-y-auto ui-scroll">
                        {sets.map((set) => (
                            <Space
                                key={set.id}
                                className={set.id === active?.id ? 'font-medium' : ''}
                            >
